...
function randomQuote() {
    ...
    fetch(api_url).then(response => response.json()).then(data => renderQuote(data));
}    
...
quoteBtn.addEventListener("click", function() {
    ...
    quoteBtn.style.background = quoteIcon.style.color = quoteText.style.color = quoteAuthor.style.color = twitter.style.color = tumblr.style.color = `${color1}`
});
